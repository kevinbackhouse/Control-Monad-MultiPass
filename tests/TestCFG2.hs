...
import Control.Monad.MultiPass.Example.CFG2
import Data.Ix
...
runEmitCFG g =
  do g' <- newST2Array_ (Node 0, Node (length g - 1))
     sequence_
       [ writeST2Array g' (Node i) (map Node ks)
       | (i,ks) <- zip [0..] g
       ]
     xs <- emitCFG (NumThreads 4) g'
